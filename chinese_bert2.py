...
if __name__ == '__main__':
    ...
    get_probability(blocking, 'result/cbert/blocking.tsv', blocking=True)
    ...
    get_probability(subj_f1, 'result/cbert/subj_f1.tsv', female_first=True)
    get_probability(subj_m1, 'result/cbert/subj_m1.tsv', female_first=False)
    print('In subject orientation in a gender-biased setting, the percentage of local binding:')
    get_probability(subj_f1_bias, 'result/cbert/subj_f1_bias.tsv', female_first=False)
    get_probability(subj_m1_bias, 'result/cbert/subj_m1_bias.tsv', female_first=True)
    print('In the local binding setting, the percentage of local binding is: ')
    get_probability(local_f1, 'result/cbert/local_f1.tsv', local_binding=True, female_first=False)
    get_probability(local_m1, 'result/cbert/local_m1.tsv', local_binding=True, female_first=True)
